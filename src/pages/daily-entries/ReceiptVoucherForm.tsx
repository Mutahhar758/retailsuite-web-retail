...
import { receiptService, type ReceiptLineRequest } from '../../services/receiptService';
...
export const ReceiptVoucherForm: React.FC = () => {
    ...
    if (isEdit) {
      const loadVoucher = async () => {
        try {
          ...
          navigate('/daily-entries/receipt-voucher');
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleAccountChange = async (accountId: string) => {
    if (!accountId) return;
    try {
      const balance = await receiptService.getAccountBalance(accountId);
  ...
  const removeRow = async (record: any) => {
    if (isEdit && record.seq) {
      try {
        await receiptService.deleteLine(voucherNo!, record.seq);
  ...
  const handleSave = async () => {
    try {
      ...
      setLoading(true);
      const lines: ReceiptLineRequest[] = validLines.map(l => ({
        seq: l.seq,
        account: l.account,
        amount: l.amount,
      ...
      if (isEdit) {
        ...
        const newVNo = await receiptService.create(request);
        message.success('Voucher created successfully: ' + newVNo);
        navigate(`/daily-entries/receipt-voucher/${newVNo}`);
      }
    } catch (error: any) {
  ...
  return (
    <Card className="shadow-sm border-gray-100 rounded-xl">
      <div className="flex justify-between items-center mb-6">
        <Space align="center">
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/daily-entries/receipt-voucher')} />
          <FileTextOutlined style={{ fontSize: 24, color: '#16a34a' }} />
          <div>
            <Title level={4} style={{ margin: 0 }}>
              {isEdit ? `Edit Receipt Voucher: RV-${voucherNo}` : 'New Receipt Voucher'}
            </Title>
            <Text type="secondary">{isEdit ? 'Modify existing receipt record' : 'Create a new cash/bank receipt record'}</Text>
          </div>
        </Space>
        <Space>
          <Button type="primary" icon={<SaveOutlined />} onClick={handleSave} loading={loading} style={{ backgroundColor: '#16a34a', borderColor: '#16a34a' }}>Save Voucher</Button>
          {isEdit && (
      ...
      <Form form={form} layout="vertical">
        ...
        <div className="flex justify-between items-center mb-4 mt-2">
          <Title level={5} style={{ margin: 0 }}>Receipt Lines</Title>
              ...
              <Table.Summary fixed>
                <Table.Summary.Row>
                  ...
                  <Table.Summary.Cell index={1}>
                    <Text type="success" strong>{totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</Text>
                  </Table.Summary.Cell>

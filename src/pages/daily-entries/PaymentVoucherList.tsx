...
export const PaymentVoucherList: React.FC = () => {
  ...
  const fetchData = useCallback(async (values: any = {}) => {
    try {
      ...
      const result = await paymentService.getList(params);
      setData(result);
    } catch (error) {
      message.error('Failed to fetch payment vouchers');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'voucherNo',
      key: 'voucherNo',
      render: (text: string) => <Tag color="blue">PV-{text}</Tag>,
    },
    {
      title: 'Narration',
      dataIndex: 'narration',
    ...
    },
    {
      ...
      render: (_: any, record: PaymentDto) => (
        <Button 
          ...
          icon={<EditOutlined />} 
          size="small"
          onClick={() => navigate(`/daily-entries/payment-voucher/${record.voucherNo}`)}
        >
          Edit
        </Button>
      ),
    ...
    <Card className="shadow-sm border-gray-100 rounded-xl">
      <div className="flex justify-between items-center mb-6">
        <Space align="center">
          ...
          <div>
            <Title level={4} style={{ margin: 0 }}>Payment Vouchers</Title>
            <Text type="secondary">Manage cash and bank payment records</Text>
          </div>
        </Space>
        <Button 
          type="primary" 
          icon={<PlusOutlined />} 
          onClick={() => navigate('/daily-entries/payment-voucher/new')}
        >
          New Payment
        </Button>
      </div>
      ...
      <Table 
        ...
        onRow={(record) => ({
          onDoubleClick: () => navigate(`/daily-entries/payment-voucher/${record.voucherNo}`)
        })}

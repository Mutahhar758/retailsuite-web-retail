...
import {
  SearchOutlined, PrinterOutlined, FileProtectOutlined,
...
export const AccountStatementWithDue: React.FC = () => {
  ...
  const [data, setData] = useState<(AccountStatementWithDueLine & { balance: number })[]>([]);
  ...
  const handleSearch = async (values: any) => {
    ...
    try {
      ...
      const res = await reportService.getAccountStatementWithDue(filter);
      ...
      message.error('Failed to load account statement with due');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'vNo',
      width: 110,
      render: (vNo: string) => vNo ? <Tag color="blue">{vNo}</Tag> : '-'
    },
    {
      title: 'Particulars',
    ...
    },
    {
      ...
      render: (days: number | null) => days !== null ? (
        <Tag color={days > 30 ? 'red' : 'orange'} icon={<ClockCircleOutlined />}>
          {days}d
        </Tag>
      ) : '-'
    },
  ...
  return (
    <Card className="shadow-sm border-gray-100 rounded-xl">
      <div className="flex justify-between items-center mb-6 no-print">
        <Space align="center">
          <FileProtectOutlined style={{ fontSize: 24, color: '#16a34a' }} />
          <div>
            <Title level={4} style={{ margin: 0 }}>Account Statement with Due</Title>
            <Text type="secondary">Detailed ledger with payment aging information</Text>
          </div>
        </Space>
                ...
                return (
                  <Table.Summary fixed>
                    <Table.Summary.Row className="bg-gray-50 font-bold">
                      <Table.Summary.Cell index={0} colSpan={4} align="right">Totals</Table.Summary.Cell>

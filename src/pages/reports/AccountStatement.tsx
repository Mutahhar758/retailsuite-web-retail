...
export const AccountStatement: React.FC = () => {
  ...
  const [currentFileName, setCurrentFileName] = useState<string>('AccountStatement.pdf');
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [selectedAccountTitle, setSelectedAccountTitle] = useState<string>('');
  const [statementData, setStatementData] = useState<(AccountStatementLine & { balance: number })[]>([]);
  ...
  const handleSearch = async (values: any) => {
    ...
    try {
      ...
      // Fetch PDF stream and raw structured data concurrently
      const [blob, rawLines] = await Promise.all([
        reportService.getAccountStatementPdf(filter),
        reportService.getAccountStatement(filter)
      ]);
      ...
      // Calculate running balances for Excel / CSV exports
      ...
      const fileName = `AccountStatement_${values.account}_${values.dateRange[0].format('YYYYMMDD')}_${values.dateRange[1].format('YYYYMMDD')}.pdf`;
      setCurrentFileName(fileName);
    } catch (error) {
      console.error('Failed to load account statement', error);
      message.error('Failed to generate Account Statement');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportCsv = () => {
    ...
    const fileName = `AccountStatement_${values.account || 'Account'}_${values.dateRange[0].format('YYYYMMDD')}_${values.dateRange[1].format('YYYYMMDD')}.csv`;

    const headers = ['Date', 'Voucher #', 'Particulars / Narration', 'Debit (Dr)', 'Credit (Cr)', 'Balance'];
    ...
    message.success('Account statement exported to CSV');
  };
  ...
  const handleExportExcel = () => {
    ...
    const values = form.getFieldsValue();
    const fileName = `AccountStatement_${values.account || 'Account'}_${values.dateRange[0].format('YYYYMMDD')}_${values.dateRange[1].format('YYYYMMDD')}.xls`;
    ...
    const excelHtml = `
      ...
      <body style="font-family: Segoe UI, Arial, sans-serif;">
        <table>
          <tr>
            <td colspan="6" style="font-size: 16px; font-weight: bold; color: #1e3a8a; padding: 8px 0;">
              ACCOUNT STATEMENT
            </td>
          </tr>
          <tr>
            <td colspan="6" style="font-size: 12px; font-weight: bold; color: #374151;">
              Account: ${selectedAccountTitle || values.account}
            </td>
          </tr>
          <tr>
            <td colspan="6" style="font-size: 11px; color: #6b7280; padding-bottom: 10px;">
          ...
          <tfoot>
            <tr style="background-color: #f3f4f6; font-weight: bold;">
              <td colspan="3" style="border: 1px solid #d1d5db; padding: 6px; text-align: right;">TOTALS & NET MOVEMENT</td>
    ...
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    message.success('Account statement exported to Excel');
  };
      ...
      >
        ...
        <div className="flex items-center justify-between pb-3 px-2 border-b border-gray-200 dark:border-gray-800 flex-shrink-0">
          <Space align="center">
            ...
            <Title level={5} style={{ margin: 0 }}>Account Statement</Title>
          </Space>
                ...
                description={
                  <div>
                    ...
                    <Text type="secondary" style={{ fontSize: 13 }}>
                      Select an account and date range on the left panel, then click <b>Show Report</b> to view the full statement.
                    </Text>
                  </div>
                }
